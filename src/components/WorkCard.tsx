...
const WorkCard = ({ work, onClick, index }: WorkCardProps) => {
  ...
  return (
    <motion.article
      ...
      onClick={onClick}
      className={`group cursor-pointer touch-manipulation transform-gpu ${work.featured ? 'sm:col-span-2 lg:col-span-2' : ''}`}
    >
      <div className="glass-card-hover h-full overflow-hidden">
        ...
        <div className={`relative overflow-hidden ${work.featured ? 'aspect-[16/9] sm:aspect-[16/10]' : 'aspect-[4/3]'}`}>
          ...
          {/* Hover Overlay - Hidden on mobile/touch devices to prevent stuck states */}
          <div className="hidden sm:block absolute inset-0 bg-gradient-to-t from-background/90 via-background/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />

          {/* Arrow Icon - Only show on hover/focus on desktop */}
          <div className="absolute top-3 right-3 sm:top-4 sm:right-4 w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-foreground/10 sm:backdrop-blur-sm flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            <ArrowUpRight size={16} className="text-foreground sm:w-[18px] sm:h-[18px]" />
          </div>

          {/* Category Badge - Solid background on mobile, blur on desktop */}
          <div className="absolute top-3 left-3 sm:top-4 sm:left-4">
            <span className="px-2.5 py-1 sm:px-3 text-xs font-medium bg-background/90 sm:bg-background/60 sm:backdrop-blur-sm rounded-full text-foreground/80 border border-white/10">
              {categoryLabel}
            </span>
          </div>
        </div>
